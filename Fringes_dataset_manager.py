...
class dataset():
    ...
    def plot(self, fmt=None, radius=None, straight=False, label=None, residuals=False, **kwargs):
        """
        ...
        plt.ylabel('Fringe number [1]')
        plt.plot(aa[mask], fa[mask], fmt, **kwargs, label=label)
...
class measure():
    ...
    def plot(self, **kwargs):
        name = self.name
        kwargs['alpha'] = 0.5
        self.background.plot(**kwargs,fmt='bo', label='Background: ' + name)
        self.signal.plot(**kwargs,fmt='r*', label='Net signal: ' + name)
        self.gross_signal.plot(**kwargs,fmt='g^', label='Gross signal: ' + name)
        plt.legend()
