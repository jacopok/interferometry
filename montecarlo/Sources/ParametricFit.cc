...
void ParametricFit::add_unknown_parameter(double min, double max, unsigned int steps, string& name){
...
PDF* ParametricFit::get_unknown_PDF(string& name) const{
...
bool ParametricFit::isready() const{
  ...
  if(f->n_arguments() != fixed_parameters->size() + unknown_parameters_name->size()){
    cout << "Fit FAILED: number of parameters doesn't match with the given function" << endl;
    return false;
  }
  if(data_x->size() == 0){
